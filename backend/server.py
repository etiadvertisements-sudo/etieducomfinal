...
try:
    ...
    LlmChat = None
    UserMessage = None
    logging.warning("emergentintegrations not available - chatbot disabled")
import httpx
import asyncio
from seo_notify import notify_search_engines, build_blog_url
from meta_capi import send_lead_event as meta_send_lead_event
...
# Uploads directory (legacy static files only)
UPLOADS_DIR = ROOT_DIR / "uploads"
UPLOADS_DIR.mkdir(exist_ok=True)

# ── Emergent Object Storage (fallback when Cloudinary is not configured) ──
import requests as _requests

STORAGE_APP_NAME = "eti-educom"
...
def init_storage(force: bool = False):
    ...
    if _STORAGE_KEY and not force:
        return _STORAGE_KEY
    resp = _requests.post(f"{STORAGE_URL}/init", json={"emergent_key": os.environ.get("EMERGENT_LLM_KEY")}, timeout=30)
...
CLOUDINARY_ENABLED = bool(os.environ.get("CLOUDINARY_CLOUD_NAME") and os.environ.get("CLOUDINARY_API_SECRET"))

# ============ Rate Limiter ============
limiter = Limiter(key_func=get_remote_address)

# ============ HTML Sanitizer for blog content (prevent stored XSS) ============
ALLOWED_HTML_TAGS = [
    ...
    'video', 'source', 'iframe', 'section', 'article', 'header', 'footer', 'nav',
    'dl', 'dt', 'dd', 'sub', 'sup', 'mark', 'abbr', 'details', 'summary'
]
ALLOWED_HTML_ATTRS = {
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate" if "/api/admin" in request.url.path else "public, max-age=60"
        return response
...
# CRM Webhook URL (set in env, never hardcoded)
...
async def send_lead_to_crm(name: str, phone: str, email: str = "", source: str = "Website", campaign: str = "", program_name: str = ""):
    """Forward lead to CRM webhook (with light retry)."""
    if not CRM_WEBHOOK_URL:
        logging.info("CRM_WEBHOOK_URL not configured; skipping CRM forwarding")
        return False
    payload = {
        "name": name,
        ...
        try:
            async with httpx.AsyncClient() as http_client:
                ...
                if response.status_code in (200, 201):
                    logging.info(f"CRM webhook sent: {name} ({source})")
                    return True
    ...
    logging.error(f"CRM webhook failed after retries: {last_error}")
    return False
...
async def create_contact_enquiry(request: Request, input: ContactEnquiryCreate):
    try:
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone or "", email=input.email, source="Contact Form", program_name=input.enquiry_type)

        # Server-side Meta Conversions API Lead event (fire-and-forget, deduped with browser pixel via event_id)
        await meta_send_lead_event(
            lead_type="contact", request=request, name=input.name, email=input.email,
            phone=input.phone, external_id=doc['id'], value=500,
            custom_fields={"enquiry_type": input.enquiry_type},
        )
...
async def get_contact_enquiries(admin: dict = Depends(get_current_admin)):
...
async def delete_contact_enquiry(contact_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_event(input: EventCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_event(event_id: str, input: EventUpdate, admin=Depends(get_current_admin)):
...
async def create_review(input: ReviewCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_review(review_id: str, input: ReviewUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def create_program(input: ProgramCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_program(program_id: str, input: ProgramUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def delete_program(program_id: str, admin=Depends(get_current_admin)):
...
async def seed_all_programs(admin=Depends(get_current_admin)):
...
async def create_job(input: JobOpeningCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_job(job_id: str, input: JobOpeningUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def create_application(input: JobApplicationCreate, request: Request = None):
    try:
        ...
        await meta_send_lead_event(
            ...
            phone=input.phone, external_id=doc['id'], value=2000,
            custom_fields={"job_id": input.job_id},
        )
...
async def create_hire_request(input: HireRequestCreate):
    try:
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.contact_person, phone=input.phone, email=input.email, source="Hire From Us", program_name=input.company_name)
...
async def get_hire_requests(admin: dict = Depends(get_current_admin)):
...
async def chat_with_ai(input: ChatMessage):
    try:
        if LlmChat is None:
            return ChatResponse(
                response="AI Chat is not available on this server. Please contact our team directly at +91 9646727676.",
...
class Blog(BaseModel):
    ...
    # Content Meta
    word_count: int = 0
    heading_structure: List[str] = []  # ["H2: Title", "H3: Subtitle"]
    ...
    scheduled_at: Optional[str] = None  # ISO timestamp; future = hidden from public
...
class BlogCreate(BaseModel):
    ...
    seo_score: int = 0
    published_at: Optional[str] = None
    scheduled_at: Optional[str] = None  # ISO timestamp; if set & in future, blog hidden from public list/detail
...
class BlogUpdate(BaseModel):
    ...
    heading_structure: Optional[List[str]] = None
    internal_links: Optional[List[str]] = None
    external_links: Optional[List[str]] = None
    seo_score: Optional[int] = None
    published_at: Optional[str] = None
    scheduled_at: Optional[str] = None
    slug: Optional[str] = None  # Allow slug change → triggers 301 redirect record
...
def _blog_to_response(b: dict) -> BlogResponse:
    """Helper to convert a blog document to BlogResponse with all SEO fields"""
    return BlogResponse(
        ...
        focus_keyword=b.get('focus_keyword'), secondary_keywords=b.get('secondary_keywords', []),
        canonical_url=b.get('canonical_url'), robots=b.get('robots', 'index, follow'),
        og_image=b.get('og_image'), og_type=b.get('og_type', 'article'),
        faq_items=b.get('faq_items', []), word_count=b.get('word_count', 0),
        heading_structure=b.get('heading_structure', []),
        internal_links=b.get('internal_links', []), external_links=b.get('external_links', []),
        seo_score=b.get('seo_score', 0), published_at=b.get('published_at'),
...
async def create_blog(input: BlogCreate, admin=Depends(get_current_admin)):
    try:
        blog_dict = input.model_dump()
        # Sanitize HTML content to prevent XSS
        blog_dict['content'] = sanitize_html(blog_dict['content'])
        blog_dict['title'] = sanitize_text(blog_dict['title'])
        blog_dict['excerpt'] = sanitize_text(blog_dict['excerpt'])
        ...
        # Auto-notify search engines (IndexNow + Google Indexing API) when published
        if doc.get('is_published'):
            asyncio.create_task(
                notify_search_engines([build_blog_url(doc['slug'])], action="URL_UPDATED")
            )

        return _blog_to_response(doc)
    except Exception as e:
...
async def get_blogs(published_only: bool = True, featured_only: bool = False, limit: int = 100):
    ...
    if published_only:
        ...
        # Hide scheduled-future blogs from public output
...
async def get_blog(blog_slug: str):
    ...
    if not blog:
        blog = await db.blogs.find_one({"id": blog_slug}, {"_id": 0})
    if not blog:
        raise HTTPException(status_code=404, detail="Blog not found")
    # Hide if scheduled in the future and not published
    sched = blog.get("scheduled_at")
    if sched and sched > datetime.now(timezone.utc).isoformat():
        if not blog.get("is_published"):
            raise HTTPException(status_code=404, detail="Blog not found")
    return _blog_to_response(blog)


@api_router.get("/blogs/redirect/{old_slug}")
async def get_blog_redirect(old_slug: str):
    """Used by Next.js to follow 301 redirects when a blog slug has changed."""
    rec = await db.slug_redirects.find_one({"old_slug": old_slug, "target_type": "blog"}, {"_id": 0})
    if not rec:
        raise HTTPException(status_code=404, detail="No redirect")
    return {"new_slug": rec["new_slug"]}


@api_router.put("/blogs/{blog_id}", response_model=BlogResponse)
async def update_blog(blog_id: str, input: BlogUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")

    # Sanitize HTML/text fields if present
    ...
    if 'excerpt' in update_data:
        update_data['excerpt'] = sanitize_text(update_data['excerpt'])

    # If slug is changing, register a 301 redirect from old slug → new slug
    if 'slug' in update_data:
        existing = await db.blogs.find_one({"id": blog_id}, {"_id": 0, "slug": 1})
        if existing and existing.get("slug") and existing["slug"] != update_data['slug']:
            await db.slug_redirects.update_one(
                {"old_slug": existing["slug"], "target_type": "blog"},
                {"$set": {
    ...
    # Auto-notify search engines whenever a published blog is updated.
    # Skip if blog is scheduled in the future.
    if blog.get('is_published'):
        sched = blog.get('scheduled_at')
        if not sched or sched <= datetime.now(timezone.utc).isoformat():
            asyncio.create_task(
                notify_search_engines([build_blog_url(blog['slug'])], action="URL_UPDATED")
            )
...
async def delete_blog(blog_id: str, admin=Depends(get_current_admin)):
    ...
    # Tell Google to drop the URL from its index
    if blog and blog.get('slug'):
        asyncio.create_task(
            notify_search_engines([build_blog_url(blog['slug'])], action="URL_DELETED")
        )

    return {"message": "Blog deleted successfully"}


# ============ SEO: Manual search-engine ping ============

class SeoPingRequest(BaseModel):
    urls: Optional[List[str]] = None  # full URLs OR plain blog slugs
    slug: Optional[str] = None        # convenience: ping a single blog slug
    action: str = "URL_UPDATED"        # or "URL_DELETED"


@api_router.post("/admin/seo/ping")
@limiter.limit("30/hour")
async def admin_seo_ping(request: Request, input: SeoPingRequest, admin=Depends(get_current_admin)):
    """Manually trigger IndexNow + Google Indexing API for one or more URLs/slugs."""
    ...
    if input.urls:
        for u in input.urls:
            ...
            if not u:
                continue
            candidate_urls.append(u if u.startswith("http") else build_blog_url(u.lstrip("/")))
    if input.slug:
        candidate_urls.append(build_blog_url(input.slug.strip().lstrip("/")))

    if not candidate_urls:
        raise HTTPException(status_code=400, detail="Provide 'urls' or 'slug'")

    result = await notify_search_engines(candidate_urls, action=input.action)
    return {"submitted": candidate_urls, "result": result}


# ============ Franchise Enquiry Routes ============

@api_router.post("/franchise-enquiry", response_model=FranchiseEnquiryResponse)
async def create_franchise_enquiry(input: FranchiseEnquiryCreate, request: Request = None):
    try:
        enquiry_dict = input.model_dump()
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, email=input.email, source="Franchise Enquiry", program_name=f"Franchise - {input.city}")

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="franchise", request=request, name=input.name, email=input.email,
            phone=input.phone, city=input.city, external_id=doc['id'], value=5000,
            custom_fields={"location": input.location, "investment_budget": input.investment_budget},
        )
...
async def get_franchise_enquiries(admin: dict = Depends(get_current_admin)):
...
async def delete_franchise_enquiry(enquiry_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_counselling_lead(input: CounsellingLeadCreate, request: Request = None):
    try:
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, source="Free Counselling", program_name=input.preferred_track)

        # Server-side Meta Conversions API Lead event
        await meta_send_lead_event(
            lead_type="counselling", request=request, name=input.name,
            phone=input.phone, external_id=doc['id'], value=800,
            custom_fields={"preferred_track": input.preferred_track, "education": input.education},
        )
...
async def get_counselling_leads(admin: dict = Depends(get_current_admin)):
...
async def delete_counselling_lead(lead_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_faq(input: FAQCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_faq(faq_id: str, input: FAQUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def upsert_seo_settings(input: SEOSettingsCreate, admin: dict = Depends(get_current_admin)):
    try:
...
async def seed_all_seo(admin=Depends(get_current_admin)):
...
async def upsert_technical_seo(input: TechnicalSEOCreate, admin: dict = Depends(get_current_admin)):
    try:
...
async def get_msg91_settings(admin: dict = Depends(get_current_admin)):
    settings = await db.msg91_settings.find_one({}, {"_id": 0})
    if not settings:
        return {
            "auth_key": "",
...
async def save_msg91_settings(input: MSG91SettingsUpdate, admin: dict = Depends(get_current_admin)):
    try:
        update_data = {}
...
async def test_msg91_whatsapp(phone: str, name: str = "Test User", admin=Depends(get_current_admin)):
...
async def update_founder_settings(input: FounderSettingsUpdate, admin: dict = Depends(get_current_admin)):
    try:
...
async def create_cyber_warriors_registration(input: CyberWarriorsRegistrationCreate, request: Request = None):
    try:
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.contact_number, email=input.email, source="Cyber Warriors", program_name="Cybersecurity Awareness")

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="cyber_warriors", request=request, name=input.name, email=input.email,
            phone=input.contact_number, external_id=doc['id'], value=700,
            custom_fields={"registration_type": input.registration_type, "organization_name": input.organization_name},
        )
...
async def get_cyber_warriors_registrations(admin: dict = Depends(get_current_admin)):
...
async def delete_cyber_warriors_registration(reg_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_cyber_warriors_event(input: CyberWarriorsEventCreate, admin: dict = Depends(get_current_admin)):
    try:
        event_dict = input.model_dump()
...
async def delete_cyber_warriors_event(event_id: str, admin: dict = Depends(get_current_admin)):
...
async def get_cyber_warriors_assessments(passed_only: bool = False, admin: dict = Depends(get_current_admin)):
...
async def delete_cyber_warriors_assessment(assessment_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_cyber_warriors_video_review(input: CyberWarriorsVideoReviewCreate, admin: dict = Depends(get_current_admin)):
    try:
        review_dict = input.model_dump()
...
async def delete_cyber_warriors_video_review(review_id: str, admin: dict = Depends(get_current_admin)):
...
async def toggle_cyber_warriors_video_review(review_id: str, admin: dict = Depends(get_current_admin)):
...
async def change_admin_password(input: ChangePasswordRequest, admin=Depends(get_current_admin)):
    if not verify_password(input.current_password, admin["password_hash"]):
        raise HTTPException(status_code=400, detail="Current password is incorrect")
    new_hash = hash_password(input.new_password)
    await db.admin_users.update_one({"id": admin["id"]}, {"$set": {"password_hash": new_hash}})
...
async def upload_file(file: UploadFile = File(...), admin=Depends(get_current_admin)):
    ...
    if CLOUDINARY_ENABLED:
        try:
            ...
            return {"url": result["secure_url"], "filename": result.get("public_id", ""), "storage": "cloudinary"}
        except Exception as e:
            logging.error(f"Cloudinary upload failed: {e}, falling back to local storage")
    
    # Fallback to Emergent object storage
    unique_name = f"{uuid.uuid4().hex}{ext}"
    storage_path = f"{STORAGE_APP_NAME}/uploads/{unique_name}"
    content_type = MIME_TYPES.get(ext.lstrip("."), "application/octet-stream")
    result = storage_put(storage_path, contents, content_type)
    ...
    return {"url": f"/api/media/{unique_name}", "filename": unique_name, "storage": "object"}


@api_router.get("/media/{filename}")
async def serve_media(filename: str):
    record = await db.uploads.find_one({"filename": filename, "is_deleted": False})
    if not record:
        raise HTTPException(status_code=404, detail="File not found")
    data, content_type = storage_get(record["storage_path"])
...
async def create_summer_training_lead(input: SummerTrainingLeadCreate, request: Request = None):
    try:
        lead_dict = input.model_dump()
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, email=input.email or "", source="Summer Training", program_name=input.program_interest)

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="summer_training", request=request, name=input.name, email=input.email,
            phone=input.phone, external_id=doc['id'], value=1000,
            custom_fields={"program_interest": input.program_interest, "duration": input.duration},
        )
...
async def get_summer_training_leads(admin: dict = Depends(get_current_admin)):
    leads = await db.summer_training_leads.find({}, {"_id": 0}).sort("created_at", -1).to_list(1000)
    return [
        SummerTrainingLeadResponse(
            id=lead['id'], name=lead['name'], email=lead.get('email'), phone=lead['phone'],
...
async def delete_summer_training_lead(lead_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_industrial_training_lead(input: IndustrialTrainingLeadCreate, request: Request = None):
    try:
        lead_dict = input.model_dump()
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, email=input.email, source="Industrial Training", program_name=input.program_interest)

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="industrial_training", request=request, name=input.name, email=input.email,
            phone=input.phone, external_id=doc['id'], value=1200,
            custom_fields={"program_interest": input.program_interest, "college": input.college},
        )
...
async def get_industrial_training_leads(admin: dict = Depends(get_current_admin)):
...
async def delete_industrial_training_lead(lead_id: str, admin: dict = Depends(get_current_admin)):
...
@limiter.limit("10/minute")
async def create_quick_enquiry(request: Request, input: QuickEnquiryCreate):
    try:
        enquiry_dict = input.model_dump()
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, email=input.email or "", source="Quick Enquiry", program_name=input.interest)

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="quick_enquiry", request=request, name=input.name, email=input.email,
            phone=input.phone, external_id=doc['id'], value=600,
            custom_fields={"interest": input.interest, "source": input.source},
        )
...
async def get_quick_enquiries(admin: dict = Depends(get_current_admin)):
...
async def delete_quick_enquiry(enquiry_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_referral(input: ReferralCreate):
    try:
        ...
        # Send referred friend to CRM
        await send_lead_to_crm(name=input.friend_name, phone=input.friend_phone, email=input.referrer_email or "", source="Referral", program_name=input.program_interest or "Referred by " + input.referrer_name)
...
async def get_referrals(admin: dict = Depends(get_current_admin)):
...
async def update_referral(referral_id: str, status: str, reward_amount: Optional[float] = None, admin: dict = Depends(get_current_admin)):
...
async def delete_referral(referral_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_service_enquiry(input: ServiceEnquiryCreate):
    try:
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.contact_person, phone=input.phone, email=input.email, source=f"Service - {service_name}", program_name=input.training_topic or service_name)
...
async def get_service_enquiries(service_type: Optional[str] = None, admin: dict = Depends(get_current_admin)):
    query = {}
    if service_type:
...
async def update_service_enquiry(enquiry_id: str, status: str, admin: dict = Depends(get_current_admin)):
...
async def delete_service_enquiry(enquiry_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_educonnect_university(name: str, logo: Optional[str] = None, order: int = 0, admin: dict = Depends(get_current_admin)):
...
async def update_educonnect_university(uni_id: str, name: Optional[str] = None, logo: Optional[str] = None, is_active: Optional[bool] = None, order: Optional[int] = None, admin: dict = Depends(get_current_admin)):
    update_data = {}
...
async def delete_educonnect_university(uni_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_educonnect_program(name: str, duration: str, type: str = "UG", admin: dict = Depends(get_current_admin)):
...
async def delete_educonnect_program(prog_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_educonnect_enquiry(input: EduConnectEnquiryCreate, request: Request = None):
    try:
        enquiry_dict = input.model_dump()
        ...
        # Send to CRM
        await send_lead_to_crm(name=input.name, phone=input.phone, source="ETI EduConnect", program_name=input.program_interest or "Distance Learning")

        # Server-side Meta CAPI Lead event
        await meta_send_lead_event(
            lead_type="educonnect", request=request, name=input.name,
            phone=input.phone, external_id=doc['id'], value=1000,
            custom_fields={"program_interest": input.program_interest, "qualification": input.qualification},
        )
...
async def get_educonnect_enquiries(admin: dict = Depends(get_current_admin)):
...
async def delete_educonnect_enquiry(enquiry_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_announcement(input: AnnouncementCreate, admin: dict = Depends(get_current_admin)):
    try:
...
async def update_announcement(announcement_id: str, input: AnnouncementUpdate, admin: dict = Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def get_popup_modal_admin(admin: dict = Depends(get_current_admin)):
...
async def create_or_update_popup_modal(input: PopupModalCreate, admin: dict = Depends(get_current_admin)):
...
async def toggle_popup_modal(admin: dict = Depends(get_current_admin)):
...
async def delete_popup_modal(admin: dict = Depends(get_current_admin)):
...
async def create_team_member(input: TeamMemberCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_team_member(member_id: str, input: TeamMemberUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def delete_team_member(member_id: str, admin=Depends(get_current_admin)):
...
async def create_branch(input: BranchCreate, admin: dict = Depends(get_current_admin)):
    try:
...
async def update_branch(branch_id: str, input: BranchUpdate, admin: dict = Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
...
async def delete_branch(branch_id: str, admin: dict = Depends(get_current_admin)):
...
async def create_partner(input: PartnerCreate, admin=Depends(get_current_admin)):
    try:
...
async def update_partner(partner_id: str, input: PartnerUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
...
async def delete_partner(partner_id: str, admin=Depends(get_current_admin)):
...
# ─── Brochure download (email gate → lead) ───
...
@app.post("/api/brochure-requests")
@limiter.limit("20/hour")
async def submit_brochure_request(request: Request, input: BrochureRequest):
    doc = {
        "id": str(uuid.uuid4()),
        "name": sanitize_text(input.name),
        "phone": input.phone,
        "email": input.email,
        "program_id": sanitize_text(input.program_id),
        "program_name": sanitize_text(input.program_name),
        "source": "brochure_download",
        "status": "new",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    await db.brochure_requests.insert_one(doc)
    # Forward to CRM + WhatsApp (best-effort; no blocking)
    asyncio.create_task(send_lead_to_crm(
        name=input.name, phone=input.phone, email=input.email or "",
        source="Brochure Download", program_name=input.program_name,
    ))
    try:
        asyncio.create_task(send_whatsapp_thank_you(input.phone, input.name, "Brochure Download"))
    except Exception as e:
        logging.warning(f"Brochure WhatsApp failed: {e}")
    return {"success": True, "id": doc["id"]}


# ═══════════════════════════════════════════════════════════════════════════
# Database indexes (created/ensured on startup)
# ═══════════════════════════════════════════════════════════════════════════
...
async def ensure_indexes():
    try:
        # Blogs
        await db.blogs.create_index("slug", unique=True, sparse=True)
        await db.blogs.create_index([("is_published", 1), ("created_at", -1)])
        await db.blogs.create_index([("category", 1), ("is_published", 1)])
        await db.blogs.create_index("author")
        # Slug redirects
        await db.slug_redirects.create_index([("old_slug", 1), ("target_type", 1)], unique=True)
        # Authors
        await db.authors.create_index("slug", unique=True)
        await db.authors.create_index("name")
        # Reviews
        await db.reviews.create_index([("is_active", 1), ("created_at", -1)])
        await db.reviews.create_index("course")
        # Review-collection drip
        await db.review_requests.create_index("token", unique=True)
        await db.review_requests.create_index("status")
        # Brochure leads
        await db.brochure_requests.create_index([("created_at", -1)])
        await db.brochure_requests.create_index("phone")
        # Career-quiz leads
        await db.career_quiz_leads.create_index([("created_at", -1)])
        await db.career_quiz_leads.create_index("phone")
        # Lead forms / general
        for col in ["contact_enquiries", "program_enquiries", "industrial_training_registrations",
                    "summer_training_registrations", "cyber_warriors_registrations",
                    "join_team_applications", "free_counselling", "franchise_applications",
                    "hire_from_us", "refer_and_earn"]:
            try:
                await db[col].create_index([("created_at", -1)])
                await db[col].create_index("phone")
            except Exception:
                pass
        # Admin users
        await db.admin_users.create_index("email", unique=True, sparse=True)
        # Events / WhatsApp logs
...
def _author_to_response(a: dict, article_count: int = 0) -> AuthorResponse:
    return AuthorResponse(
        id=a["id"], slug=a["slug"], name=a["name"], title=a.get("title"),
        bio=a.get("bio"), photo_url=a.get("photo_url"),
        credentials=a.get("credentials", []), expertise=a.get("expertise", []),
        linkedin_url=a.get("linkedin_url"), twitter_url=a.get("twitter_url"),
        email=a.get("email"), is_active=a.get("is_active", True),
        article_count=article_count,
    )


@app.post("/api/authors", response_model=AuthorResponse)
async def create_author(input: AuthorCreate, admin=Depends(get_current_admin)):
    slug = _slugify(input.name)
    # Ensure unique slug
    if await db.authors.find_one({"slug": slug}):
        slug = f"{slug}-{uuid.uuid4().hex[:6]}"
    author_obj = Author(slug=slug, **input.model_dump())
    doc = author_obj.model_dump()
    doc["created_at"] = doc["created_at"].isoformat()
    await db.authors.insert_one(doc)
    return _author_to_response(doc, 0)


@app.get("/api/authors", response_model=List[AuthorResponse])
async def list_authors(active_only: bool = True):
    query = {"is_active": True} if active_only else {}
    items = await db.authors.find(query, {"_id": 0}).sort("name", 1).to_list(200)
    out = []
    for a in items:
        cnt = await db.blogs.count_documents({"author": a["name"], "is_published": True})
        out.append(_author_to_response(a, cnt))
    return out


@app.get("/api/authors/{slug}", response_model=AuthorResponse)
async def get_author(slug: str):
    a = await db.authors.find_one({"slug": slug, "is_active": True}, {"_id": 0})
    if not a:
        raise HTTPException(status_code=404, detail="Author not found")
    cnt = await db.blogs.count_documents({"author": a["name"], "is_published": True})
    return _author_to_response(a, cnt)


@app.get("/api/authors/{slug}/articles")
async def get_author_articles(slug: str, limit: int = 50):
    a = await db.authors.find_one({"slug": slug, "is_active": True}, {"_id": 0})
    if not a:
        raise HTTPException(status_code=404, detail="Author not found")
    blogs = await db.blogs.find(
        {"author": a["name"], "is_published": True},
        {"_id": 0},
...
async def update_author(author_id: str, input: AuthorUpdate, admin=Depends(get_current_admin)):
    update_data = {k: v for k, v in input.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No data to update")
    if input.name:
        update_data["slug"] = _slugify(input.name)
    res = await db.authors.update_one({"id": author_id}, {"$set": update_data})
    if res.matched_count == 0:
        raise HTTPException(status_code=404, detail="Author not found")
    a = await db.authors.find_one({"id": author_id}, {"_id": 0})
    return _author_to_response(a, 0)


@app.delete("/api/authors/{author_id}")
async def delete_author(author_id: str, admin=Depends(get_current_admin)):
...
# ─── Review aggregate stats (for honest schema markup) ───
@app.get("/api/reviews/stats")
async def reviews_stats():
    """Returns average rating + count, computed from the actual reviews collection.
    Used by JSON-LD aggregateRating so we never lie to Google about review count."""
    pipeline = [
    ...
    r = rows[0]
    return {
        "average_rating": round(r.get("avg") or 0, 2),
        "review_count": int(r.get("count") or 0),
    }


# ─── Review collection drip (admin sends, student submits via token) ───
class ReviewRequest(BaseModel):
    ...
    completed_at: Optional[str] = None
    status: str = "sent"  # sent | completed | expired
...
class ReviewRequestBatchCreate(BaseModel):
    requests: List[ReviewRequestCreate]


class ReviewSubmit(BaseModel):
    rating: int = Field(..., ge=1, le=5)
    review_text: str = Field(..., min_length=10, max_length=1500)
    photo_url: Optional[str] = None


@app.post("/api/admin/review-requests")
@limiter.limit("20/hour")
async def admin_create_review_requests(
    request: Request,
    body: ReviewRequestBatchCreate,
    admin=Depends(get_current_admin),
):
    """Admin batch-creates review-collection requests. Sends WhatsApp link to each student."""
    site_url = os.environ.get("SITE_PUBLIC_URL", "https://www.etieducom.com").rstrip("/")
    created = []
    for r in body.requests:
        rr = ReviewRequest(**r.model_dump())
        doc = rr.model_dump()
        doc["sent_at"] = doc["sent_at"].isoformat()
        await db.review_requests.insert_one(doc)
        created.append({"id": doc["id"], "token": doc["token"], "name": doc["student_name"]})

        # Best-effort WhatsApp notification with link
        link = f"{site_url}/share-your-experience/{doc['token']}"
        try:
            await send_whatsapp_thank_you(
                r.phone,
                f"{r.student_name}, share your experience on ETI Educom: {link}",
                "review_request",
            )
        except Exception as e:
            logging.warning(f"Review-request WhatsApp failed for {r.phone}: {e}")
...
async def get_review_request(token: str):
    rr = await db.review_requests.find_one({"token": token}, {"_id": 0})
    if not rr:
        raise HTTPException(status_code=404, detail="Invalid or expired review link")
    if rr.get("status") == "completed":
        return {"status": "completed", "message": "Thank you — your review has already been received."}
    return {
        "status": "open",
        "student_name": rr["student_name"],
        "course": rr["course"],
    }


@app.post("/api/review-requests/{token}/submit")
@limiter.limit("5/hour")
async def submit_review_via_token(token: str, body: ReviewSubmit, request: Request):
    rr = await db.review_requests.find_one({"token": token}, {"_id": 0})
    if not rr:
        raise HTTPException(status_code=404, detail="Invalid or expired review link")
    if rr.get("status") == "completed":
    ...
    review_doc = {
        ...
        "student_name": rr["student_name"],
        "course": rr["course"],
        "review_text": sanitize_text(body.review_text),
        "photo_url": body.photo_url,
        "rating": body.rating,
        "is_active": False,  # admin moderation queue
        "source": "drip_collected",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    ...
    return {"success": True, "message": "Thank you for your review! It will appear after moderation."}


# ─── Career-quiz lead endpoint ───
class CareerQuizSubmission(BaseModel):
    name: str = Field(..., min_length=2, max_length=100)
    phone: str = Field(..., min_length=10)
    email: Optional[str] = None
    answers: dict
...
@app.post("/api/career-quiz/submit")
@limiter.limit("15/hour")
async def submit_career_quiz(request: Request, input: CareerQuizSubmission):
    doc = {
        "id": str(uuid.uuid4()),
        "name": sanitize_text(input.name),
        "phone": input.phone,
    ...
    await db.career_quiz_leads.insert_one(doc)
    # Forward to CRM + WhatsApp
    program_names = ", ".join(input.recommended_programs[:3])
    await send_lead_to_crm(
        name=input.name, phone=input.phone, email=input.email or "",
        source="Career Quiz", program_name=program_names,
    )
    await send_whatsapp_thank_you(input.phone, input.name, "Career Quiz")
    return {"success": True, "id": doc["id"]}
...
async def seed_admin_user():
    """Seed admin user with bcrypt hashed password on startup"""
    admin = await db.admin_users.find_one({"role": "admin"}, {"_id": 0})
    if not admin:
        admin_id = str(uuid.uuid4())
        hashed = hash_password(ADMIN_PASSWORD)

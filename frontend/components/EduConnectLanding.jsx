...
const defaultUniversities = [
  { id: '1', name: 'Lovely Professional University', logo: null },
  { id: '2', name: 'Manipal University Jaipur', logo: null },
  { id: '3', name: 'Amity University', logo: null },
  { id: '4', name: 'Chandigarh University', logo: null },
  { id: '5', name: 'Jain University', logo: null },
  { id: '6', name: 'NMIMS University', logo: null },
  { id: '7', name: 'Sharda University', logo: null },
  { id: '8', name: 'Sikkim Manipal University', logo: null },
];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        redirectToThankYou('educonnect', { name: formData.name });
        return;
      } else {
          ...
          <div className="flex animate-marquee">
            {[...universities, ...universities].map((uni, i) => (
              ...
              >
                {uni.logo ? (
                  <Image
                    src={cloudImg(uni.logo, 'logo')}
                    alt={uni.name}
                    width={100}
                    height={50}
                    unoptimized
                    className="max-h-12 w-auto object-contain opacity-70 hover:opacity-100 transition-opacity"
                    onError={(e) => { const p = e.target.parentNode; e.target.style.display = 'none'; if (p && !p.querySelector('.uni-fallback')) { const s = document.createElement('span'); s.className = 'uni-fallback text-xs text-gray-500 text-center font-medium'; s.textContent = uni.name; p.appendChild(s); } }}
                  />
                ) : (

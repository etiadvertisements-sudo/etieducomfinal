...
export default function HeroForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/quick-enquiry`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        trackHeroFormSubmission({ interest: formData.interest }, { name: formData.name, phone: formData.phone });
        const { redirectToThankYou } = await import('@/lib/track');
        redirectToThankYou('enquiry', { name: formData.name, program: formData.interest });
        return;
      } else {
